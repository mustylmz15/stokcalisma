...
export const useAuthStore = defineStore('auth', {
    state: () => ({
        ...
        currentProjectId: null, // Aktif proje ID'si
        userProjectRoles: {}, // Kullanıcının projeler bazındaki rolleri
    ...
    getters: {
        ...
        canManageRepairProcesses(state) {
        ...
        canApproveAfterProjectAdmin(state) {
        ...
        canSendMaterialAfterApproval(state) {
            // İleride aktifleştirilecek - Şu an için yalnızca rol kontrolü yapılıyor
            return this.isAdmin || this.isProjectManager;
        },
        ...
        canCreateITDeviceRepairRequests(state) {
            // İleride aktifleştirilecek - Şu an için yalnızca rol kontrolü yapılıyor
            return this.isAdmin || this.isProjectAdmin || this.isProjectITManager;
        },
    ...
    actions: {
        ...
        async hasProjectRole(projectId, roles) {
            if (!this.userInfo)
                return false;
        ...
        async hasActiveProjectRole(roles) {
        ...
        async hasDepotAccess(depotId, projectId) {
            if (!this.userInfo)
                return false;
        ...
        async addUserToProject(userId, projectId, projectRole = 'user') {
            try {
                // Önce yetki kontrolü
                if (!this.userInfo)
                    return false;
        ...
        async configureProjectProduct(projectId, productSettings) {
            try {
                // Önce yetki kontrolü
                if (!this.userInfo)
                    return false;
        ...
        async approveMaterialRequest(requestId, approverName, approvedItems, notes) {
            try {
                // Yetki kontrolü
        ...
        async createCentralWarehouseTransfer(transferData) {
            try {
                // Yetki kontrolü
