...
export default {
    ...
    async getLastWeekFaultyStats() {
        ...
        try {
            ...
            // Bu hafta sorgusu
            let currentWeekQuery = query(
                collection(db, 'faultyProducts'),
                where('sendDate', '>=', sevenDaysAgoTimestamp),
                where('sendDate', '<', todayTimestamp),
            ...
            currentWeekSnapshot.forEach((doc) => {
                const faultyProduct = { id: doc.id, ...doc.data() } as FaultyProductRecord;
                const sendDate = (faultyProduct.sendDate as Timestamp).toDate();
