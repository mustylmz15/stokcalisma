...
// Proje bazlı ürün ayarları arayüzü
export interface ProjectProductSettings {
    id?: string;
    projectId: string;             // Ürünün bağlı olduğu proje
    productId: string;             // İlişkilendirilmiş ana ürün ID
    isConsignment: boolean;        // Konsinye ürün mü?
    trackSerialNumber: boolean;    // Seri numarası takibi yapılıyor mu?
    repairTimeInDays: number;      // Onarım süresi (gün)
    isActive: boolean;             // Ürün aktif mi?
    createdAt: string;             // Oluşturulma tarihi
    updatedAt: string;             // Son güncelleme tarihi
    minStock?: number;             // Minimum stok seviyesi (opsiyonel)
    maxStock?: number;             // Maximum stok seviyesi (opsiyonel)
    reorderLevel?: number;         // Yeniden sipariş noktası (opsiyonel)
    notes?: string;                // Notlar (opsiyonel)
}

class ProjectProductService {
    private readonly collectionName = 'projectProductSettings';

    // Proje bazlı tüm ürün ayarlarını getir
    async getProjectProductSettings(projectId: string): Promise<ProjectProductSettings[]> {
        try {
            const q = query(
    ...
    async getProductSetting(projectId: string, productId: string): Promise<ProjectProductSettings | null> {
        try {
            const q = query(
                collection(db, this.collectionName),
                where('projectId', '==', projectId),
    ...
    async addProductSetting(setting: ProjectProductSettings): Promise<ProjectProductSettings> {
        try {
    ...
    async updateProductSetting(id: string, updatedData: Partial<ProjectProductSettings>): Promise<ProjectProductSettings> {
        try {
            if (!id) {
    ...
    async deleteProductSetting(id: string): Promise<boolean> {
        try {
            if (!id) {
