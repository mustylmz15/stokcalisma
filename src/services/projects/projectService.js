...
class ProjectService {
    ...
    userProjectsCollection = collection(db, 'userProjects');
    // Tüm projeleri getir
    async getProjects() {
        try {
    ...
    async deleteProject(id) {
        try {
            ...
            const userProjectsSnapshot = await getDocs(userProjectsQuery);
            const deletePromises = userProjectsSnapshot.docs.map(document => deleteDoc(doc(this.userProjectsCollection, document.id)));
    ...
    async addUserToProject(userId, projectId, role) {
        try {
            ...
            const userProjectQuery = query(this.userProjectsCollection, where('userId', '==', userId));
            const querySnapshot = await getDocs(userProjectQuery);
            // Bu kullanıcının bu projesi var mı kontrol et
            const existingUserProject = querySnapshot.docs.find(doc => doc.data().projectId === projectId);
    ...
    async updateUserProjectRole(userId, projectId, newRole) {
        try {
            const userProjectQuery = query(this.userProjectsCollection, where('userId', '==', userId), where('projectId', '==', projectId));
            const querySnapshot = await getDocs(userProjectQuery);
    ...
    async getWarehousesByProjectId(projectId) {
        try {
    ...
    // Depoyu projeye ekle
    async addWarehouseToProject(warehouseId, projectId) {
        try {
    ...
    async removeWarehouseFromProject(warehouseId, projectId) {
        try {
            // İlişkiyi bul
            const relationQuery = query(collection(db, 'projectWarehouses'), where('projectId', '==', projectId), where('warehouseId', '==', warehouseId));
            const relationSnapshot = await getDocs(relationQuery);
    ...
    async getProjectIdByWarehouse(warehouseId) {
        try {
            ...
            for (const projectDoc of projectsSnapshot.docs) {
                ...
                if (projectData.warehouses && Array.isArray(projectData.warehouses)) {
                    const warehouse = projectData.warehouses.find((w) => w.id === warehouseId);
                    if (warehouse) {
                ...
                if (projectData.stocks && Array.isArray(projectData.stocks)) {
                    const stock = projectData.stocks.find((s) => s.warehouseId === warehouseId);
                    if (stock) {
                        return projectDoc.id;
                    }
                }
