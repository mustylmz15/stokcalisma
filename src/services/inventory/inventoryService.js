...
const inventoryService = {
    ...
    async getAllProducts() {
        ...
        try {
            ...
            try {
                ...
                return await handleFirebasePermissionError(error, fetchProducts);
            } catch (retryError) {
                console.error('Ürün verilerini getirme hatası sonrası yeniden deneme başarısız:', retryError);
                // Hata durumunda boş array dön, böylece uygulama çalışmaya devam edebilir
                return [];
            }
        }
    },
    ...
    async getAllCategories() {
        ...
        try {
            ...
            try {
                // Yetki hatası durumunda token yenilemeyi dene
                return await handleFirebasePermissionError(error, fetchCategories);
            } catch (retryError) {
                console.error('Kategori verilerini getirme hatası sonrası yeniden deneme başarısız:', retryError);
                // Hata durumunda boş array dön, böylece uygulama çalışmaya devam edebilir
                return [];
            }
        }
    ...
    async getAllStocks() {
        ...
        try {
            ...
            try {
                // Yetki hatası durumunda token yenilemeyi dene
                return await handleFirebasePermissionError(error, fetchStocks);
            } catch (retryError) {
                console.error('Stok verilerini getirme hatası sonrası yeniden deneme başarısız:', retryError);
                // Hata durumunda boş array dön
                return [];
            }
        }
    ...
    async getAllMovements() {
        ...
        try {
            return await fetchMovements();
        } catch (error) {
            console.error('Get all movements error:', error);
            try {
                // Yetki hatası durumunda token yenilemeyi dene
                return await handleFirebasePermissionError(error, fetchMovements);
            } catch (retryError) {
                console.error('Hareket verilerini getirme hatası sonrası yeniden deneme başarısız:', retryError);
                // Hata durumunda boş array dön
                return [];
            }
        }
