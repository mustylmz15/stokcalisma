...
export default {
    ...
    async getRepairedItemsForWarehouse(statuses = ['repair_completed', 'in_transit', 'received', 'approved']) {
        try {
            ...
            let repairedItemsQuery = query(collection(db, 'repairedItems'), where('warehouseId', 'in', userWarehouses), where('status', 'in', statuses), orderBy('repairCompletedDate', 'desc'));
   ...
   */
    async getRepairedItemsByWarehouse(warehouseId) {
        try {
            const q = query(collection(db, 'repairedItems'), where('warehouseId', '==', warehouseId), orderBy('repairCompletedDate', 'desc'));
   ...
   */
    async receiveRepairedItem(itemId, notes) {
        const authStore = useAuthStore();
        try {
   ...
   */
    async approveOrRejectRepair(itemId, isApproved, notes) {
        const authStore = useAuthStore();
        try {
            const itemRef = doc(db, 'repairedItems', itemId);
            ...
            if (isApproved && data.serialNumber) {
                ...
                await serializedInventoryService.updateSerializedItemStatus(data.serialNumber, 'active', 'Onarım tamamlandı ve onaylandı');
            }
            return true;
        }
        catch (error) {
   ...
   */
    async installRepairedItem(itemId, location, notes, removeFromStock) {
        const authStore = useAuthStore();
        try {
            const itemRef = doc(db, 'repairedItems', itemId);
            const itemSnap = await getDoc(itemRef);
            ...
            if (removeFromStock) {
                ...
                if (data.serialNumber) {
                    const serializedInventoryService = (await import('@/services/inventory/serializedInventoryService')).default;
                    await serializedInventoryService.updateSerializedItemStatus(data.serialNumber, 'installed', `${location} konumuna montaj yapıldı`);
                }
                // Stoktan düş
                const inventoryService = (await import('@/services/inventory/inventoryService')).default;
                await inventoryService.processStockMovement(data.productId, data.warehouseId, 1, // Tek bir ürün
                'out', `Montaj: ${location}`, data.projectId);
            }
            return true;
        }
        catch (error) {
   ...
   */
    async updateRepairedItemStatus(itemId, status, data) {
        const authStore = useAuthStore();
        try {
            const itemRef = doc(db, 'repairedItems', itemId);
            await updateDoc(itemRef, {
