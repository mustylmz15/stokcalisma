...
import DynamicRoleService, { Role } from './dynamicRoleService';
import DynamicPermissionService, { Permission } from './dynamicPermissionService';
...
class PermissionMigrationService {
     ...
     */
    async migrateAllRolesAndPermissions(): Promise<boolean> {
        try {
            ...
            for (const roleKey of roles) {
                ...
                if (!existingRole) {
                    ...
                    await DynamicRoleService.createRole({
                        name,
                        description,
                        priority: roleHierarchy[roleKey as StaticUserRole] || 0, 
    ...
    private async migratePermissions(): Promise<void> {
        try {
            ...
            const permissionCategories: Record<string, StaticPermission[]> = {
                'Genel': [],
                'Arıza Yönetimi': [],
                'Envanter': [],
                'Proje': [],
     ...
     */
    private async migrateRolePermissions(): Promise<void> {
        try {
     ...
     */
    private formatPermissionName(permissionKey: string): string {
...
export async function migrateStaticRolesAndPermissionsToDynamic() {
    ...
    for (const permissionKey of Object.keys(StaticPermission)) {
        const permissionId = StaticPermission[permissionKey as keyof typeof StaticPermission];
        await DynamicPermissionService.createPermission({
            name: permissionId,
    ...
    for (const roleKey of Object.keys(StaticUserRole)) {
        const roleId = StaticUserRole[roleKey as keyof typeof StaticUserRole];
        await DynamicRoleService.createRole({
            name: roleId,
            description: roleId + ' rolü',
            priority: roleHierarchy[roleId as keyof typeof roleHierarchy] || 0,
            isSystem: true
        });
    }
