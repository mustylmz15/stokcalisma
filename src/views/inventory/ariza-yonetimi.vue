...
// project-store yerine düzgün çalışan projects store'unu kullanıyoruz
...
const fetchData = async () => {
    ...
    try {
        // Sırayla tüm verileri yüklemeye çalış
        await Promise.all([
            arizaStore.initializeStore(),
            inventoryStore.initializeStore(),
            projectStore.initializeStore() // projects.ts store'unu kullanarak projeleri yükle
        ]);
        
        console.log("Veriler başarıyla yüklendi");
        console.log("Depolar:", inventoryStore.getWarehouses);
        console.log("Projeler:", projectStore.projects);
        console.log("Arızalı Ürünler:", arizaStore.getFaultyProducts);
        
        // Form için başlangıç değerlerini ayarla
        if (authStore.userInfo?.role === 'user' || authStore.userInfo?.role === 'depo_sorumlusu') {
            if (authStore.userInfo.depot) {
                // Kullanıcının kendi deposunu seçili hale getir
                formData.value.senderWarehouseId = authStore.userInfo.depot;
            }
        }
    } catch (err) {
...
// Depoları kullanıcı yetkisine göre filtreleme
const warehouses = computed(() => {
    ...
    console.log("Depo filtreleme - Tüm depolar:", allWarehouses);
    console.log("Depo filtreleme - Kullanıcı:", currentUser);
    
    // Hiç depo yoksa veya depo listesi boşsa, boş dizi döndür
    if (!allWarehouses || allWarehouses.length === 0) {
        console.warn("Depo listesi boş veya tanımsız!");
        return [];
    }
    
    // Kullanıcı bilgisi yoksa tüm depoları göster
    if (!currentUser) {
        console.warn("Kullanıcı bilgisi bulunamadı, tüm depolar gösteriliyor");
        return allWarehouses;
    }
    
    // Admin veya proje admini tüm depoları görebilir
    // Konsolda gördüğümüz kadarıyla kullanıcının rolü "user", bu yüzden "user" rolünü de kontrol ediyoruz
    if (currentUser.role === 'admin' || currentUser.role === 'proje_admin') {
        return allWarehouses;
    }
      // Depo sorumlusu kendi deposunu görebilir
    // Kullanıcı "depo_sorumlusu" değil "user" olarak kaydedilmiş, bu yüzden user rolü için de depot kontrolü yapıyoruz
    if (currentUser.role === 'depo_sorumlusu' || currentUser.role === 'user') {
        // Kullanıcı deposu tanımlı değilse tüm depoları göster
        if (!currentUser.depot) {
            console.warn("Kullanıcı için depo tanımlı değil, tüm depolar gösteriliyor");
            return allWarehouses;
        }
        
        // ID eşleştirme sorununa dikkat ederek filtreleme yapalım
        console.log("Kullanıcı deposu:", currentUser.depot);
        console.log("Mevcut depo ID'leri:", allWarehouses.map(w => w.id));
        
        // Daha esnek bir eşleştirme için karşılaştırmayı geliştiriyoruz
        const userWarehouses = allWarehouses.filter(warehouse => {
            const warehouseId = String(warehouse.id).trim().toLowerCase();
            const userDepotId = String(currentUser.depot).trim().toLowerCase();
            
            return warehouseId === userDepotId || 
                  warehouseId === userDepotId.replace(/[^a-zA-Z0-9]/g, '') ||
                  warehouse.code === userDepotId; // Eğer depolarda code özelliği varsa kontrol et
        });
        
        console.log("Filtrelenmiş depolar:", userWarehouses);
        
    // Eğer kullanıcının deposu listede varsa onu döndür
        if (userWarehouses.length > 0) {
            return userWarehouses;
        } else {
            console.warn("Kullanıcı deposu depo listesinde bulunamadı! ID:", currentUser.depot);
            console.log("Mevcut tüm depo ID'leri:", allWarehouses.map(w => w.id));
            
            // Kullanıcının deposu bulunamadıysa, güvenli bir yaklaşım olarak
            // hiçbir depo göstermeyelim
            return [];
        }
    }
    
    // Diğer kullanıcı tipleri için tüm depoları göster
    return allWarehouses;
});

// Projeleri kullanıcı rolüne göre filtreleme
const projects = computed(() => {
    // projects.ts store'undaki userProjects özelliğini kullan
    // Bu özellik zaten kullanıcı yetkilerine göre filtrelenmiş projeleri döndürür
    const userAccessibleProjects = projectStore.userProjects;
    
    console.log("Kullanıcının erişebileceği projeler:", userAccessibleProjects);
    
    // Bu projeler boşsa ya da tanımlı değilse, projects.value kontrol et
    if (!userAccessibleProjects || userAccessibleProjects.length === 0) {
        const allProjects = projectStore.projects;
        if (Array.isArray(allProjects) && allProjects.length > 0) {
            console.log("Kullanıcı yetkileri filtrelenmedi, tüm projeler döndürülüyor:", allProjects);
            return allProjects;
        }
    } else {
    ...
    // En kötü durumda sabit projeler döndür
    const sabitProjeler = [
        { id: "proje1", name: "Proje 1" },
        { id: "proje2", name: "Proje 2" },
        { id: "proje3", name: "Proje 3" }
    ];
    
    console.log("Hiç proje bulunamadı, sabit projeler kullanılıyor");
    return sabitProjeler;
});

// Duruma göre filtrelenmiş ürünler
const filteredProducts = computed(() => {
    const currentUser = authStore.userInfo;
    let filteredList = faultyProducts.value || [];
    
    console.log("Arızalı ürünler (filtreleme öncesi):", filteredList.length);
    
    // Kullanıcı bilgisi yoksa hiçbir ürün gösterme
    if (!currentUser) {
        console.warn("Kullanıcı bilgisi bulunamadı, ürünler gizleniyor");
        return [];
    }
    
    // Admin veya proje admini tüm ürünleri görebilir
    if (currentUser.role === 'admin' || currentUser.role === 'proje_admin') {
        console.log("Admin/Proje Admin kullanıcısı, tüm ürünler gösteriliyor");
    } 
    // Depo sorumlusu veya user rolündeki kullanıcı sadece kendi deposuna ait ürünleri görebilir
    else if ((currentUser.role === 'depo_sorumlusu' || currentUser.role === 'user') && currentUser.depot) {
        console.log("Depo sorumlusu filtresi uygulanıyor. Kullanıcı deposu:", currentUser.depot);
        
        const userDepotId = String(currentUser.depot).trim().toLowerCase();
        
        // ID karşılaştırması yaparken birden fazla format kontrolü yap
        filteredList = filteredList.filter(product => {
            if (!product.senderWarehouseId) {
                return false;
            }
            
            const productDepotId = String(product.senderWarehouseId).trim().toLowerCase();
            const isMatch = productDepotId === userDepotId || 
                         productDepotId.replace(/[^a-zA-Z0-9]/g, '') === userDepotId.replace(/[^a-zA-Z0-9]/g, '');
            
            if (isMatch) {
                console.log(`Ürün eşleşti: ${product.serialNumber} - Depo: ${product.senderWarehouseId}`);
            }
            
            return isMatch;
        });
        
        console.log("Depoya göre filtrelenmiş ürün sayısı:", filteredList.length);
    } 
    // Diğer kullanıcılar için, hiçbir ürün görmemeliler
    else {
        console.log("Yetkisiz kullanıcı, ürünler gizleniyor");
        return [];
    }
    ...
    if (selectedStatus.value) {
        ...
        filteredList = filteredList.filter(p => p.status === selectedStatus.value);
        console.log(`Durum filtrelemesi: ${selectedStatus.value}, önceki: ${beforeCount}, sonraki: ${filteredList.length}`);
    }
...
onMounted(() => {
    console.log("Auth Store on mount:", authStore.userInfo);
    fetchData();
    
    // Kullanıcı bilgilerini kontrol et
    if (!authStore.userInfo) {
        console.warn("Kullanıcı bilgileri yüklenemedi, filtreleme doğru çalışmayabilir");
    }
});
</script>
