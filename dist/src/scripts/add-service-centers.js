...
async function checkExistingServiceCenters() {
    try {
...
async function main() {
    ...
    for (const center of serviceCentersToAdd) {
        const exists = existingCenters.some(existing => existing.name.toLowerCase() === center.name.toLowerCase());
        if (exists) {
...
//# sourceMappingURL=add-service-centers.js.map