...
import { collection, doc, setDoc, addDoc, getDoc, getDocs, updateDoc, deleteDoc, query, where, serverTimestamp } from 'firebase/firestore';
...
export const inventoryService = {
        ...
        try {
            ...
            if (projectId !== undefined) {
                try {
                    ...
                    const relationQuery = query(collection(db, 'projectWarehouses'), where('warehouseId', '==', warehouseId));
    ...
    async addMovement(movementData) {
        try {
            ...
            if (movementData.type === 'in') {
                // Stok girişi
                await this.processStockMovement(movementData.productId, movementData.targetWarehouseId || movementData.sourceWarehouseId, movementData.quantity, 'in', movementData.description || '', movementData.targetProjectId || movementData.sourceProjectId // Proje bilgisini ekledik
                );
            }
            else if (movementData.type === 'out') {
                // Stok çıkışı
                await this.processStockMovement(movementData.productId, movementData.sourceWarehouseId, movementData.quantity, 'out', movementData.description || '', movementData.sourceProjectId // Proje bilgisini ekledik
                );
            }
            else if (movementData.type === 'transfer' && movementData.targetWarehouseId) {
                // Depolar arası transfer
                await this.processStockMovement(movementData.productId, movementData.sourceWarehouseId, movementData.quantity, 'out', `Transfer: ${movementData.sourceWarehouseId} → ${movementData.targetWarehouseId}`, movementData.sourceProjectId // Kaynak proje bilgisini ekledik
                );
                await this.processStockMovement(movementData.productId, movementData.targetWarehouseId, movementData.quantity, 'in', `Transfer: ${movementData.sourceWarehouseId} → ${movementData.targetWarehouseId}`, movementData.targetProjectId || movementData.sourceProjectId // Hedef proje bilgisini ekledik
                );
            }
            else if (movementData.type === 'stock_add' && movementData.targetWarehouseId) {
                // Stok ekleme (dışarıdan) - sadece hedef depoya ekleme yap
                await this.processStockMovement(movementData.productId, movementData.targetWarehouseId, movementData.quantity, 'in', `Stok Ekleme: Test Depo → ${movementData.targetWarehouseId}`, movementData.sourceProjectId // Kaynak proje bilgisini kullan
                );
            }
            return {
                id: docRef.id,
    ...
    async getWarehousesByProject(projectId) {
        try {
            ...
            const projectWarehousesQuery = query(collection(db, 'projectWarehouses'), where('projectId', '==', projectId));
    ...
    async addWarehouseToProject(projectId, warehouseId) {
        try {
            ...
            const relationQuery = query(collection(db, 'projectWarehouses'), where('projectId', '==', projectId), where('warehouseId', '==', warehouseId));
            const relationSnapshot = await getDocs(relationQuery);
    ...
    async getMovementsByProject(projectId) {
        try {
            ...
            for (const warehouseId of warehouseIds) {
                ...
                const sourceMovementsQuery = query(movementsRef, where('sourceWarehouseId', '==', warehouseId));
                ...
                const targetMovementsQuery = query(movementsRef, where('targetWarehouseId', '==', warehouseId));
    ...
    async getStocksByProject(projectId) {
        try {
            ...
            for (const warehouseId of warehouseIds) {
                const stocksQuery = query(stocksRef, where('warehouseId', '==', warehouseId));
