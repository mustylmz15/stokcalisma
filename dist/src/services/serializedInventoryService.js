...
import { collection, doc, setDoc, addDoc, getDoc, getDocs, updateDoc, deleteDoc, query, where, Timestamp, serverTimestamp } from 'firebase/firestore';
...
const serializedInventoryService = {
    ...
    async addSerializedItem(itemData) {
        try { // Seri numarası kontrolü
            ...
            const serialQuery = query(collection(db, 'serializedItems'), where('serialNumber', '==', itemData.serialNumber));
            ...
            const stockQuery = query(collection(db, 'stocks'), where('productId', '==', itemData.productId), where('warehouseId', '==', itemData.warehouseId));
    ...
    async getSerializedItemBySerialNumber(serialNumber) {
        try {
            const serialQuery = query(collection(db, 'serializedItems'), where('serialNumber', '==', serialNumber));
            const serialSnapshot = await getDocs(serialQuery);
            if (serialSnapshot.empty) {
    ...
    async getSerializedItemsByProduct(productId) {
        try {
            const serialQuery = query(collection(db, 'serializedItems'), where('productId', '==', productId));
            const serialSnapshot = await getDocs(serialQuery);
            const items = [];
    ...
    async getSerializedItemsByWarehouse(warehouseId) {
        try {
            const serialQuery = query(collection(db, 'serializedItems'), where('warehouseId', '==', warehouseId));
            const serialSnapshot = await getDocs(serialQuery);
            const items = [];
            serialSnapshot.forEach((doc) => {
                items.push({
...
//# sourceMappingURL=serializedInventoryService.js.map